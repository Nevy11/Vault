  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center">
        ...
        <p className="mt-4 text-muted-foreground animate-pulse font-medium">Locating Merchant Vault...</p>
      </div>
    );
  }
  ...
  if (!merchant) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-muted-foreground mb-8">The username @{cleanUsername} does not appear to be a valid Vault OS account.</p>
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in duration-500">
        ...
        <p className="text-muted-foreground mb-12 max-w-sm">
          You have successfully sent <span className="text-foreground font-semibold">${parseFloat(amount).toLocaleString()}</span> to <span className="text-foreground font-semibold">{merchant.business?.business_name || merchant.first_name}</span>.
        </p>
      ...
      <main className="relative z-10 max-w-screen-xl mx-auto px-6 py-12 lg:py-24 grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-16 items-center">
        ...
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left space-y-12">
          <div className="space-y-6">
            ...
            <h1 className="text-5xl lg:text-7xl font-black tracking-tight text-balance leading-none">
              Pay <span className="text-primary">{merchant.business?.business_name || merchant.first_name}</span> instantly.
            </h1>
            <p className="text-xl text-muted-foreground max-w-lg leading-relaxed">
              Scan the dynamic QR code or use the link below to send secure funds directly to {merchant.kyc_tag}.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-8 w-full max-w-lg">
            ...
            <div className="flex-1 space-y-4 w-full">
               <div className="space-y-2">
                 <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold ml-1">Direct Payment Link</Label>
               ...
               <div className="flex items-center gap-3 p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10">
                  ...
                  <div className="text-[10px] text-muted-foreground leading-tight">
                    Funds are settled <span className="text-emerald-500 font-bold">Instantly</span> in the merchant's vault with Zero-Trust protection.
                  </div>
               </div>
            </div>
          </div>
        ...
        <div className="w-full">
           <div className="bg-card/40 border border-border/50 rounded-[48px] p-10 backdrop-blur-2xl shadow-2xl space-y-10 relative overflow-hidden group">
              ...
              <div className="flex items-center gap-5 p-6 rounded-3xl bg-background/40 border border-border/40 shadow-inner relative z-10">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xl font-bold truncate">{merchant.business?.business_name || `${merchant.first_name} ${merchant.last_name}`}</div>
                  <div className="text-sm text-primary font-mono">{merchant.kyc_tag}</div>
                  <div className="flex items-center gap-1.5 mt-1">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Verified Vault Business</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-8 relative z-10">
                <div className="space-y-4">
                  <Label className="text-xs uppercase tracking-widest text-muted-foreground font-black ml-1">Payment Method</Label>
                  <div className="grid grid-cols-2 gap-3 p-1.5 bg-background/60 border border-border/60 rounded-3xl">
                    <button 
                      ...
                      className={cn(
                        ...
                        method === "vault" ? "bg-primary text-primary-foreground shadow-lg shadow-primary/20" : "text-muted-foreground hover:bg-white/5"
                      )}
                    >
                    ...
                    <button 
                      ...
                      className={cn(
                        "flex items-center justify-center gap-2 py-4 rounded-[20px] text-sm font-bold transition-all",
                        method === "mpesa" ? "bg-emerald-600 text-white shadow-lg shadow-emerald-500/20" : "text-muted-foreground hover:bg-white/5"
                      )}
                    >
                ...
                <div className="space-y-4">
                  <Label className="text-xs uppercase tracking-widest text-muted-foreground font-black ml-1">Payment Amount (USD)</Label>
                  <div className="relative group">
                    <span className="absolute left-6 top-1/2 -translate-y-1/2 text-4xl font-light text-muted-foreground/40 group-focus-within:text-primary transition-colors">$</span>
                    <Input 
                      type="number"
                      placeholder="0.00"
                  ...
                  <div className="flex justify-between items-center px-2">
                    <div className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Available Rails</div>
                    <div className="flex gap-2 opacity-40 group-hover:opacity-100 transition-opacity">
                      <div className="w-6 h-6 rounded-md bg-white border p-1"><img src="https://upload.wikimedia.org/wikipedia/commons/b/ba/Stripe_Logo%2C_revised_2016.svg" className="w-full h-full object-contain" /></div>
                      <div className="w-6 h-6 rounded-md bg-emerald-600 p-1 flex items-center justify-center text-[8px] font-black text-white">M</div>
                    </div>
                  </div>
                </div>
                ...
                <Button 
                  ...
                  className={cn(
                    ...
                    method === "vault" ? "bg-primary hover:bg-primary/90 shadow-primary/30" : "bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/30"
                  )}
                >
                  {status === "processing" ? (
                    ...
                    <>
                      Confirm Payment <ArrowRight className="ml-3 w-6 h-6 group-hover:translate-x-2 transition-transform" />
                    </>
                  )}
                </Button>
                ...
                <div className="flex items-center justify-center gap-4 text-[10px] text-muted-foreground font-bold uppercase tracking-[0.2em] pt-4 border-t border-border/40">
                  <div className="flex items-center gap-1.5"><Lock className="w-3 h-3 text-emerald-500" /> SSL</div>
                  <div className="flex items-center gap-1.5"><ShieldCheck className="w-3 h-3 text-emerald-500" /> AES-256</div>
                  <div className="flex items-center gap-1.5"><Zap className="w-3 h-3 text-emerald-500" /> INSTANT</div>
                </div>
              </div>
           </div>
           ...
           <div className="mt-8 flex items-center justify-center gap-2 text-xs text-muted-foreground">
             ...
             New to Vault? <Link to="/sign-up" className="text-primary font-bold hover:underline">Create your secure wallet in 60 seconds.</Link>
           </div>
        </div>
      </main>
    </div>
