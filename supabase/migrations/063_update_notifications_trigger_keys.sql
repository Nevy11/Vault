-- Update handle_transaction_notification to use translation keys
...
-- Update handle_activity_notification to use translation keys
...
BEGIN
    ...
    IF NEW.action_type = 'login' THEN
        ...
        VALUES (
            NEW.user_id,
            'New Account Login', -- Fallback
            'Your account was accessed from a ' || COALESCE(NEW.device_info, 'new device') || ' in ' || COALESCE(NEW.location, 'Unknown Location') || '.', -- Fallback
            'nav.notifications.login_title',
            'nav.notifications.login_msg',
            jsonb_build_object(
                'device', COALESCE(NEW.device_info, 'new device'),
                'location', COALESCE(NEW.location, 'Unknown Location')
            ),
            'info'
        );
    END IF;
    ...
    IF NEW.is_suspicious = true THEN
        ...
        VALUES (
            ...
            'A suspicious activity was detected on your account: ' || NEW.action_type || '. If this wasn''t you, please change your PIN immediately.', -- Fallback
            'nav.notifications.security_alert_title',
            'nav.notifications.security_alert_msg',
            jsonb_build_object('action', NEW.action_type),
            'warning'
        );
    END IF;
