-- Migration: Fix Automated Reminders and Savings Process to handle potential nulls and division by zero
    ...
    FOR v_loan IN SELECT * FROM public.loans WHERE status = 'active' LOOP
        ...
        IF v_last_reminder IS NULL OR v_last_reminder <= NOW() - INTERVAL '3 days' THEN
            ...
            VALUES (
                ...
                'You have KES ' || COALESCE(v_loan.remaining_balance, 0) || ' left on your loan. Continue to pay it will increase your limit and trust score!',
                'warning',
    ...
    FOR v_goal IN SELECT * FROM public.savings_goals WHERE status = 'active' LOOP
        ...
        IF v_last_reminder IS NULL OR v_last_reminder <= NOW() - INTERVAL '3 days' THEN
            ...
            VALUES (
                ...
                'You are at ' || ROUND((COALESCE(v_goal.current_amount, 0) / COALESCE(NULLIF(v_goal.target_amount, 0), 1)) * 100) || '% of your ' || v_goal.title || ' goal. Continue saving to hit your target and earn rewards!',
                'info',
    ...
    FOR v_goal IN SELECT * FROM public.savings_goals WHERE status = 'active' AND is_automated = true LOOP
        IF NOT EXISTS (
            ...
            BEGIN
                ...
                IF v_balance >= COALESCE(v_goal.automation_amount, 0) AND COALESCE(v_goal.automation_amount, 0) > 0 THEN
                    -- 1. Deduct from wallet
                    UPDATE public.wallets SET balance = balance - v_goal.automation_amount WHERE id = v_wallet_id;

                    -- 2. Add to savings goal
                    UPDATE public.savings_goals SET current_amount = COALESCE(current_amount, 0) + v_goal.automation_amount WHERE id = v_goal.id;
                    ...
                    VALUES (v_goal.id, v_goal.user_id, v_goal.automation_amount, 'Vault Account', 'automated', COALESCE(v_goal.current_amount, 0) + v_goal.automation_amount);
                ...
                ELSIF COALESCE(v_goal.automation_amount, 0) > 0 THEN
