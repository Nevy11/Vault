-- Migration: Fix Receipt Trigger Security
-- Description: Sets the receipt generation function to SECURITY DEFINER to bypass RLS during automated generation.

BEGIN;

-- 1. Update the Trigger Function to be SECURITY DEFINER
-- This allows the function to insert into 'receipts' using system-level permissions,
-- bypassing the need for users to have direct INSERT permissions on the receipts table.
...
BEGIN
    ...
    -- For withdrawals/transfers/savings: sender_id is the user
...
-- 2. Add System-level RLS Policies for Receipts
-- Ensure users can view their own receipts (Select)
