-- Migration: Fix pay_bill_split Method Null Error
-- Description: Redefines the pay_bill_split function to be more robust and explicitly handle the transaction method.

BEGIN;

-- 1. Ensure the 'vault' method exists in the enum
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 2. Redefine the payment RPC with explicit column names and fallback logic
...
BEGIN
    ...
    -- Log transaction with extremely explicit values
    ...
    VALUES (
        ...
        COALESCE('vault'::public.transaction_method, 'bank'::public.transaction_method),
        v_amount,
