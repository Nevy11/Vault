...
serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      ...
      return new Response(JSON.stringify({ error: "INTERNAL_CONFIG_ERROR" }), { status: 500, headers: corsHeaders });
    }
    ...
    if (!email) {
      ...
      return new Response(JSON.stringify({ error: "EMAIL_NOT_FOUND" }), { status: 400, headers: corsHeaders });
    }
    ...
    if (wallet && Number(wallet.balance) > 0) {
      return new Response(JSON.stringify({ error: "Account balance must be zero before deletion." }), { 
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      })
    }
    ...
    if (activeLoans && activeLoans.length > 0) {
      return new Response(JSON.stringify({ error: "All loans must be repaid fully before deletion." }), { 
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      })
    }
    ...
    if (activeSavings && activeSavings.length > 0) {
      return new Response(JSON.stringify({ error: "All savings must be withdrawn before deletion." }), { 
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      })
    }
    ...
    return new Response(JSON.stringify({ success: true, message: "Confirmation email sent via Magic Link template" }), { 
      headers: { ...corsHeaders, "Content-Type": "application/json" } 
    })
  } catch (error: any) {
