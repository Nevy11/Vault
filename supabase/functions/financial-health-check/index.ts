...
serve(async (req) => {
  ...
  try {
    ...
    const { data: insightData, error: insertError } = await supabase.from("financial_insights").insert({
      user_id: userId,
      type: insightType,
    ...
    if (insightJson.severity === "high" || insightJson.severity === "medium" || insightType === "alert" || insightType === "prediction") {
